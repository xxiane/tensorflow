...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
  ...
  for sub_dir in sub_dirs:
    ...
    if dir_name == image_dir:
      continue
    tf.logging.info("Looking for images in '" + dir_name + "'. Acceptable extensions are jpg, jpeg, JPG, JPEG, png.")
...
def add_decoding(input_width, input_height, input_depth, input_mean,
                      input_std, format):
  ...
  Returns:
    Tensors for the node to feed JPEG data into, and the output of the
      preprocessing steps.
  """
  image_data = tf.placeholder(tf.string, name='DecodeInput')

  if (format=="png"):
    decoded_image = tf.decode_png(jpeg_data, channels=input_depth)
  else:
    decoded_image = tf.image.decode_jpeg(jpeg_data, channels=input_depth)
  decoded_image_as_float = tf.cast(decoded_image, dtype=tf.float32)
  decoded_image_4d = tf.expand_dims(decoded_image_as_float, 0)
...
def main(_):
  ...
  with tf.Session(graph=graph) as sess:
    ...
    jpeg_data_tensor, decoded_image_tensor = add_decoding(
